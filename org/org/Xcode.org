...
2 为工程运行时加入 NSZombieEnabled、 MallocStackLogging环境变量，并设为启用。
    produce-->edit schema-->run (environment variables)中

3 为工程运行时加入 NSZombieEnabled 环境变量，并设为启用。
4 MallocStackLogging 启用
